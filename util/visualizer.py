...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for idx, real_img in enumerate(real_imgs):
        fake_img = fake_imgs[idx]
        s = structural_similarity(real_img, fake_img, multichannel=True, data_range=real_img.max())
        ssim.append(s)
        p = peak_signal_noise_ratio(real_img, fake_img, data_range=real_img.max())
        psnr.append(p)
